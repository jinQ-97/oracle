...
--ANY(SOME) : 메인쿼리의 조건식을 만족하는 서브쿼리의 결과가 하나 이상이면 TRUE반환
-- =ANY(SOME) = IN
...
--ALL : 서브쿼리의 모든 결과가 조건식이 맞아 떨어져야만 메인 쿼리의 조건식이 TRUE가 된다
-- < ALL == MIN()
...
SELECT * FROM EMP e WHERE  EXISTE (SELECT d.DNANE FROM DEPT d WHERE D.DEPTNO = 10)

--전체 사원 중 ALLEN과 같은 직책인 사원들의 사원정보,부서정보 조회 (사번,이름,직무,부서명 출력)

SELECT E.EMPNO, E.NAME, E.JOB, D.DNAME
FROM EMP e JOIN DEPT d ON E.DEPTNO = D. DEPTNO
WHERE E.JOB = (SELECT E2.JOB FROM EMP e2 WHERE E2.ENAME = 'ALLEN') 

--10번 부서에 근무하는 사원 중 30번 부서에 없는 직책을 가진 사원의 사번,이름,직무,부서번호,부서명,부서위치 조회
...
-- DEPT_TMP 테이블에 50번 부서 추가
INSERT INTO DEPT_TMP(DEPTNO,DNAME,LOC)
VALUES (50,'DATABACE','SEOUL')
...
-- NULL 삽입이 가능한 컬럼의 값은 NULL을 명시하거나
INSERT INTO DEPT_TMP(DEPT_NO, DNAME, LOC)
VALUES (50,'WEB',NULL)
-- 넣지 않아도 된다 ( NULL 암시적 처리 )
INSERT INTO DEPT_TMP(DEPT_NO, DNAME)
VALUES (50, 'OS')
...
INSERT INTO EMP_TMP (EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, DEPTNO)
VALUES (3333,'성춘향','MANAGER', 9999, SYSDATE, 4000, NULL, 20)
...
--DEPT 테이블의 40번 부서의 DNAM, LOC를 가져와서 DEPT_TEMP 50번 부서의 내용으로 변경 
UPDATE DEPT_TMP dt
...
CREATE EXAM_DEFT AS SELECT * FROM DEFT
CREATE EXAM_EMP AS SELECT * FROM EMP
CREATE EXAM_SALGRADE AS SELECT * FROM SALGRADE
...
INSERT INTO EXAM_DEFT --생략( , , )
VALUES (50, 'ORACLE', 'BUSAN'),(60, 'SQL', 'ILSAN'),(70, 'SELECT', 'INCHEON'),(80, 'DML', 'BUNDANG')

--EXAM_EMP 테이블에 임의의 사원 정보 삽입 (5명)
INSERT INTO EXAM_EMP (EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, DEPTNO)
VALUES (3333,'성춘향','MANAGER', 9999, SYSDATE, 4000, NULL, 20)

--EXAM_EMP 에 속한 사원 중 50번 부서에서 근무하는 사원의 평균 급여보다 많이 받는 사원을 70번 부서로 옮기는 SQL 구문 작성

UPDATE EXAM_EMP ee
SET ee.DEPT_NO = 70
WHERE EE.SAL > (SELECT AVG(E.SAL) FROM EXAM_EMP WHERE DEPTNO=50)

-- EXAM_EMP 에 속한 사원 중 입사일이 가장 빠른 60번 부서 사원보다 늦게 입사한 사원의 급여를 10%인상하고 80번 부서로 옮기는 SQL구문 작성
UPDATE EXAM_EMP ee
SET ee.SAL = EE.SAL*1.1,EE.DEPTNO = 80
WHERE EE.HIREDATE > (SELECT MIN(E.HIREDATE) FROM EXAM_EMP WHERE DEPTNO=60) 
...
--트랜젝션 : 더 이상 분활할 수 없는 최소 수행 단위 (한번에 처리해야하는)
--CREATE,UPDATE,DELETE : 데이터에 변경을 줄 수 있는 작업
...
--세션 : 데이터베이스 접속을 시작으로 관련작업을 수행한 후 접속을 종료하기까지의 전체 기간  [ DBEVER 세션  / SQL PLUS 세션]
 
DELETE FROM DEPT_TCL WHERE DEPTNO = 50
COMMIT

UPDATE DEPT_TCL dt SET DT.LOC = 'SEOUL' WHERE DT.DEPTNO = 30
COMMIT 
...
--	컬럼명2 타입,
--)
-- 테이블명은 문자로 시작 (숫자,특수문자[_,$,#],한글)\

-- 한 서버 내 동일한 테이블명은 존재 불가
-- 예약어는 테이블 이름으로 사용불가 (ORDER)
-- 컬럼명은 문자로 시작
-- 하나의 테이블 안에서 컬럼명 중복 불가
-- 예약어는 컬럼 이름으로 사용불가
-- 자료형
-- 숫자 NUMBER (전체자릿수,소수점이하자릿수) EX. NUMBER(7,2), NUMBER(8)
-- 문자 VARCHAR2(N바이트) : 가변길이 문자열 EX. VARCHAR2(20)  : 영어 20문자, 한글 6문자
-- 		CHAR(N바이트) : 고정길이 문자열 EX. CHAR(20) 
-- 		NCHAR(N개) : 고정길이 유니코드 문자열 NCHAR(20) : 한글,영어 모두 20문자
--	NVARCHAR2(N개) : 가변길이 유니코드 문자열
-- 날짜 : DATE, TIMESTAMP
-- 대용량 데이터 
--		CLOB :	대용량 문자 데이터
-- 		BLOB : 	대용량 바이너리 데이터
